...
function toHistoryTurn(msg: ChatMessageModel): HistoryTurn {
	...
	return {
		...
		createdAt: msg.createdAt,
		verified: msg.result.verified,
		degraded: msg.result.degraded,
...
function getHeaderTraceId(res: Response): string | undefined {
	return (res.headers.get('x-trace-id') ?? res.headers.get('cf-trace-id') ?? res.headers.get('trace-id') ?? undefined)?.toString();
}

async function postChatWithScope(req: ChatRequestWithScope): Promise<AnswerResult> {
	const document_ids = uniqueStrings(req.documentIds ?? []);
		...
		return hist.map((t: StoredTurn) =>
			t.role === 'user'
				...
				: ({
						...
						role: 'assistant',
						createdAt: t.createdAt,
						result: { answer: t.content, citations: t.citations ?? [], verified: t.verified ?? false, degraded: t.degraded ?? false } satisfies AnswerResult,	
					} as const),
		);
	});
  ...
  try {
	...
	async function send(text: string, attachments: UploadItem[] = []) {
		...
		try {
			...
			const document_ids = uniqueStrings([...activeDocumentIds, ...attachments.map((u) => u.documentId)]);
							...
							{messages.length === 0 ? (
								<div className="rounded-xl border border-slate-800 bg-slate-900/30 p-6">
									...
									<div className="mt-2 text-sm text-slate-400">
										Your <span className="font-mono">session_id</span> is persisted in localStorage and passed to the backend so the Worker
										can load prior turns.
									</div>
								</div>
							) : null}
